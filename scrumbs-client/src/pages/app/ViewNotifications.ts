

export class ViewNotifications {
    public static readonly SWITCH_TO_SCRUM_VIEW: string                         = "SWITCH_TO_SCRUM_VIEW";
    public static readonly SWITCH_TO_IMPEDIMENTS_VIEW: string                   = "SWITCH_TO_IMPEDIMENTS_VIEW";
    public static readonly SWITCH_TO_REPORTS_VIEW: string                       = "SWITCH_TO_REPORTS_VIEW";
    public static readonly SWITCH_HEADER_STATE: string                          = "SWITCH_HEADER_STATE";
    public static readonly SWITCH_TO_MANAGE_TEAMS_VIEW: string                  = "SWITCH_TO_MANAGE_TEAMS_VIEW";
    public static readonly SWITCH_TO_CREATE_TEAM_VIEW: string                   = "SWITCH_TO_CREATE_TEAM_VIEW";
    public static readonly SWITCH_TO_CREATE_IMPEDIMENT_VIEW: string             = "SWITCH_TO_CREATE_IMPEDIMENT_VIEW";
    public static readonly SWITCH_TO_ACCOUNT_SETTINGS_VIEW: string              = "SWITCH_TO_ACCOUNT_SETTINGS_VIEW";

    public static readonly SWITCH_TO_PREVIOUS_VIEW: string                      = "SWITCH_TO_PREVIOUS_VIEW";

    public static readonly INIT_ONBOARDING_MEMBER_EDIT_FLOW: string             = "INIT_ONBOARDING_MEMBER_EDIT_FLOW";
    public static readonly INIT_ONBOARDING_IMPEDIMENT_FEATURE_FLOW: string      = "INIT_ONBOARDING_IMPEDIMENT_FEATURE_FLOW";

    public static readonly CLEAR_ONBOARDING: string                             = "CLEAR_ONBOARDING";
}